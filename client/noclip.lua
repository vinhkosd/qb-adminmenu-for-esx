...
local NO_CLIP_NORMAL_SPEED = 0.35
local NO_CLIP_FAST_SPEED = 0.7
...
local function SetNoClip(val)
    if (isNoClipping ~= val) then
        ...
        if (isNoClipping) then
            ...
            SetEntityAlpha(noClippingEntity, currentAlpha, 0)
            ...
            CreateThread(function()
                ...
                while isNoClipping do
                    ...
                    SetEntityAlpha(clipped, currentAlpha, false)
...
function ToggleNoClipMode(alpha)
    currentAlpha = 0
    if alpha ~= nil then
        currentAlpha = alpha
    end
