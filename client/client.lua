...
local menu_button3 = menu:AddButton({
    icon = '🎮',
    label = 'Quản lý Server(time/weather)',
    value = menu5,
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('pepe-weathersync:server:setWeather', selection.value)
            exports['okokNotify']:Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords3' then
        ...
        exports['okokNotify']:Notify("Đã copy toạ độ!", "success")
    ...
    elseif dataType == 'heading' then
        ...
        exports['okokNotify']:Notify("Đã copy heading!", "success")
    end
end
...
local function OpenPermsMenu(permsply)
    ESX.TriggerServerCallback('qb-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
			            ...
			            exports['okokNotify']:Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        exports['okokNotify']:Notify('Choose a group!', 'error')
                    end
                end
            })
        else
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                ...
                exports['okokNotify']:Notify('You must give a reason!', 'error')
            end
        end
    })
end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                ...
                exports['okokNotify']:Notify('Bạn cần cung cấp lý do và đặt thời gian ban!', 'error')
            end
        end
    })
end
...
local function OpenPlayerMenus(player)
    ...
    local elements = {
        [1] = {
            ...
            label = "Giết",
            value = "kill",
            description = "Giết người chơi " .. player.cid
        },
        [2] = {
            icon = '🏥',
            label = "Hồi sinh",
            value = "revive",
            description = "Hồi sinh người chơi " .. player.cid
        },
        [3] = {
            icon = '🥶',
            label = "Đóng băng",
            value = "freeze",
            description = "Đóng băng người chơi" .. player.cid
        },
        [4] = {
            icon = '👀',
            label = "Xem",
            value = "spectate",
            description = "Xem người chơi " .. player.cid
        },
        [5] = {
            icon = '➡️',
            label = "Teleport",
            value = "goto",
            description = "Teleport đến người chơi " .. player.cid .. ""
        },
        [6] = {
            icon = '⬅️',
            label = "Kéo",
            value = "bring",
            description = "Kéo người chơi " .. player.cid .. " tới vị trí của mình"
        },
        [7] = {
            icon = '🚗',
            label = "Sit in vehicle",
            value = "intovehicle",
            description = "Ngồi trong xe của người chơi " .. player.cid .. ""
        },
        [8] = {
            icon = '🎒',
            label = "Mở túi đồ",
            value = "inventory",
            description = "Mở túi đồ người chơi " .. player.cid .. ""
        },
        [9] = {
            icon = '👕',
            label = "Mở menu quần áo",
            value = "cloth",
            description = "Mở menu quần áo người chơi " .. player.cid
        },
        [10] = {
            icon = '🥾',
            label = "Kick",
            value = "kick",
            description = "Kick người chơi " .. player.cid .. ""
        },
        [11] = {
            icon = '🚫',
            label = "Ban",
            value = "ban",
            description = "Cấm người chơi " .. player.cid .. " khỏi server"
        },
        [12] = {
            icon = '🎟️',
            label = "Permissions",
            value = "perms",
            description = "Set quyền cho " .. player.cid .. ""
        }
    }
    for k, v in ipairs(elements) do
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("pepe-weathersync:server:setTime", value, value)
    exports['okokNotify']:Notify("Time changed to " .. value .. " hs 00 min")
...
menu_button6:On('select', function(item)
    TriggerEvent('pepe-hospital:client:revive', PlayerPedId())
end)
